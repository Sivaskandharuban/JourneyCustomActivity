...
app.post("/api/createperson", function (req, res) {       
    ...
    .then(function (data) {       
        ...
        .catch(function (err) {       
            ...
            // print the error;     return res.status(400).json({ success: false, error: err });     })     .finally(function () {       pgp.end(); 
                // for immediate app exit, closing the connection pool.     }); });

// app.post("/api/updatelead", function (req, res) 
//     {     
//         var updateQuery =      "UPDATE lead SET company = 'PashtekUS' WHERE company = 'Pashtek'";
...
//         console.log("ERROR:", err); 
//         // print the error;        
//         return res.status(400).json({ success: false, error: err });      })      .finally(function () {        pgp.end(); // for immediate app exit, closing the connection pool.      });  });    app.post("/api/createlead", function (req, res) {        const { name, company } = req.body;    var insertQuery =      "INSERT INTO lead (name, company) VALUES ('" +      name +      "','" +      company +      "')";
//     db.query(insertQuery, true)      
//     .then(function (data) {        
//         return res.json(data);      
//     })      
...
//     var deleteQuery =      "DELETE FROM lead WHERE company = 'PashtekUS'";
//     db.query(deleteQuery, true)      .then(function (data) {        return res.json(data);      })           .catch(function (err) {        console.log("ERROR:", err); // print the error;        return res.status(400).json({ success: false, error: err });      })      .finally(function () {        pgp.end(); // for immediate app exit, closing the connection pool.      });  });
